...
import {
  uploadImageDirectToOCI,
  uploadMultipleFilesDirectToOCI,
...
const EditProject = () => {
  ...
  const [customManufacturerLogoFile, setCustomManufacturerLogoFile] =
    useState(null)
  const [customManufacturerLogoPreview, setCustomManufacturerLogoPreview] =
    useState("")
  ...
  const handleProjectChange = async (event, newValue) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (projectData) {
          // Use API visibility value directly.
          if (
            projectData.isVisible !== undefined &&
            projectData.isVisible !== null &&
            projectData.isVisible !== ""
          ) {
            setIsVisible(safeVisibilityBoolean(projectData.isVisible))
          } else {
            setIsVisible(false)
          }
          ...
          const newFormData = {
            ...
            SCL:
              extractFilePaths(
                typeof projectData.SCL === "string"
                  ? JSON.parse(projectData.SCL)
                  : projectData.SCL,
              ) ?? formData.SCL,
            pro_plan:
              extractFilePaths(
                typeof projectData.pro_plan === "string"
                  ? JSON.parse(projectData.pro_plan)
                  : projectData.pro_plan,
              ) ?? formData.pro_plan,
          ...
          if (projectData.logo) {
            ...
            const isStaticLogo = staticLogoOptions.some(
              (logo) => logo.url === logoValue,
            )
          ...
          if (projectData.manufactureLogo) {
            ...
            const isStaticManufacturerLogo = staticLogoOptions.some(
              (logo) => logo.url === manufacturerLogoValue,
            )
            if (
              !isStaticManufacturerLogo &&
              typeof manufacturerLogoValue === "string" &&
              manufacturerLogoValue.startsWith("http")
            ) {
          ...
          const isNoNRC =
            (projectData.amount === "0" || projectData.amount === 0) &&
            (projectData.amount_rem === "0" || projectData.amount_rem === 0)
          setNoNRC(isNoNRC)
  ...
  const safeVisibilityBoolean = (value) => {
    ...
    try {
      ...
      if (Array.isArray(formData.SCL) && formData.SCL.length > 0) {
        ...
        if (newSCLFiles.length > 0) {
          try {
            toast.loading(`Uploading ${newSCLFiles.length} SCL file(s)...`, {
              id: "scl-upload",
            })
            const uploadedFiles = await uploadMultipleFilesDirectToOCI(
              newSCLFiles,
              "scl",
            )
            sclFiles.push(...uploadedFiles)
            toast.success("SCL files uploaded successfully!", {
              id: "scl-upload",
            })
          } catch (error) {
            toast.error("Failed to upload SCL files: " + error.message, {
              id: "scl-upload",
            })
            setIsSubmitting(false)
            return
      ...
      if (Array.isArray(formData.pro_plan) && formData.pro_plan.length > 0) {
        ...
        if (newPlanFiles.length > 0) {
          try {
            toast.loading(`Uploading ${newPlanFiles.length} PLAN file(s)...`, {
              id: "plan-upload",
            })
            const uploadedFiles = await uploadMultipleFilesDirectToOCI(
              newPlanFiles,
            ...
            toast.success("PLAN files uploaded successfully!", {
              id: "plan-upload",
            })
          } catch (error) {
            toast.error("Failed to upload PLAN files: " + error.message, {
              id: "plan-upload",
            })
            setIsSubmitting(false)
            return
      ...
      const jsonData = {
        ...
        cont_deliverable_total: safeNumberString(
          formData.cont_deliverable_total,
        ),
        fixture: formData.fixture || "",
        files: formData.files
          ? isNaN(Number(formData.files))
            ? 0
      ...
      if (error.response?.status === 400) {
        ...
        toast.error(
          "Project with this title already exists. Please choose a different title.",
        )
  ...
  const mainLogoPreviewSrc =
    customLogoPreview ||
    (typeof formData.logo === "string" && formData.logo
      ? getOciUrl(formData.logo) || formData.logo
      : "")
  const manufacturerLogoPreviewSrc =
    customManufacturerLogoPreview ||
    (typeof formData.manufactureLogo === "string" && formData.manufactureLogo
      ? getOciUrl(formData.manufactureLogo) || formData.manufactureLogo
      : "")

  const isMainStaticLogoSelected =
    typeof formData.logo === "string" && formData.logo
      ? staticLogoOptions.some((logo) => logo.url === formData.logo)
      : false
  const isManufacturerStaticLogoSelected =
    typeof formData.manufactureLogo === "string" && formData.manufactureLogo
      ? staticLogoOptions.some((logo) => logo.url === formData.manufactureLogo)
      : false

  const activeLogoContext = logoModalContext ?? "main"
  const activeLogoValue =
    activeLogoContext === "manufacturer"
      ? formData.manufactureLogo
      : formData.logo
  const activeIsCustomLogoSelected =
    activeLogoValue &&
    !staticLogoOptions.some((logo) => logo.url === activeLogoValue)
  const activeCustomPreviewSrc =
    (activeLogoContext === "main"
      ? customLogoPreview
      : customManufacturerLogoPreview) ||
    (activeIsCustomLogoSelected &&
    typeof activeLogoValue === "string" &&
    activeLogoValue
      ? getOciUrl(activeLogoValue) || activeLogoValue
      : "")
            ...
            <div className="bg-white rounded-xl shadow-2xl p-10 flex flex-col items-center gap-6 min-w-[300px]">
              <div className="relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div
                    className="w-12 h-12 border-4 border-transparent border-t-blue-400 rounded-full animate-spin"
                    style={{
          ...
          <div className="p-4 md:p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div
                    ...
                    className={`border-2 rounded-lg p-4 text-center cursor-pointer transition-all duration-200 min-h-[140px] flex flex-col items-center justify-center ${
                      formData.logo
                  ...
                  >
                    {mainLogoPreviewSrc ? (
                      <div className="space-y-3 w-full">
                        ...
                        <p className="text-xs text-gray-500">
                          {isMainStaticLogoSelected
                            ? "Static logo selected"
                  ...
                  >
                    {manufacturerLogoPreviewSrc ? (
                      <div className="space-y-3 w-full">
                        ...
                        <p className="text-xs text-gray-500">
                          {isManufacturerStaticLogoSelected
                            ? "Static logo selected"
                            : "Custom logo selected"}
                        </p>
                      </div>
                ...
                <div>
                  ...
                  <FormControl fullWidth size="small">
                    <Select
                      ...
                      onChange={(e) =>
                        handleChange({
                          target: { name: "risk_sts", value: e.target.value },
                        })
                      }
                      renderValue={(value) => {
                        return renderStatusIcon(value)
            ...
            <div className="bg-gradient-to-br from-gray-50 to-white rounded-lg p-4 border border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div>
                  <label
                    htmlFor="cont_deliverable_total"
                    className="block text-xs font-semibold text-gray-700 mb-1.5"
                  >
                    Total Deliverables
                  </label>
                ...
                <div>
                  ...
                  <FormControl fullWidth size="small">
                    <Select
                      ...
                      onChange={(e) =>
                        handleChange({
                          target: { name: "amount_sts", value: e.target.value },
                        })
                      }
                      disabled={noNRC}
                      renderValue={(value) => {
            ...
            <div className="overflow-y-auto p-6">
              ...
              <div className="mb-8 border-2 border-dashed border-gray-300 rounded-xl p-6 bg-gradient-to-br from-gray-50 to-white hover:border-blue-400 transition-colors">
                ...
                >
                  ...
                  <span className="text-sm font-semibold text-gray-700 mb-1">
                    {logoModalContext === "manufacturer"
                      ? "Upload Custom End Customer Logo"
                      : "Upload Custom Logo"}
                  </span>
              ...
              <div>
                ...
                {isLogoListLoading ? (
                  ...
                  <p className="text-center text-gray-500 py-8">
                    No logos available. Please contact administrator.
                  </p>
                ) : (
                    ...
                    {staticLogoOptions.map((logo) => {
                      ...
                      return (
                        <div
                          key={logo.id}
                          onClick={() =>
                            handleStaticLogoSelect(logo.url, logoModalContext)
                          }
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full overflow-hidden">
            ...
            <div className="p-6">
              <p className="text-gray-600 text-center mb-6">
                Are you sure you want to update the project "{formData.title}"?
                This action will save all changes.
              </p>
              <div className="flex gap-3 justify-center">
                <button
                  type="button"
