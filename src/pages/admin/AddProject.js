...
const AddProject = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.SCL && formData.SCL.length > 0) {
        toast.loading(`Uploading ${formData.SCL.length} SCL file(s)...`, {
          id: "scl-upload",
        })
      ...
      if (formData.pro_plan && formData.pro_plan.length > 0) {
        toast.loading(`Uploading ${formData.pro_plan.length} PLAN file(s)...`, {
          id: "plan-upload",
        })
        planFiles = await uploadMultipleFilesDirectToOCI(
          formData.pro_plan,
          "plan",
        )
  ...
  const mainLogoPreviewSrc =
    customLogoPreview ||
    (formData.logo ? getOciUrl(formData.logo) || formData.logo : "")
